...
allEqual [] = True
allEqual [x] = True
allEqual (x:y:_rest) = if x == y then allEqual (y:_rest) else False
...
rangeOf [] = 0
rangeOf (x:[]) = 0
rangeOf (x:xs) = abs ((fst p)-(snd p)) where p = extremum x x xs

extremum :: (Ord a, Num a) => a->a->[a]->(a,a)
extremum small big [] = (small, big)
extremum small big (x:xs) = extremum (min x small) (max x big) xs
...
longest (x:[]) = x
longest (x:y:_rest)
        | length x < length y = longest (y:_rest)
        | length x > length y = longest (x:_rest)
        | otherwise = if x < y then longest (x:_rest) else longest (y:_rest)
...
incrementKey d [] = []
incrementKey d ((a,b) : _rest)
            | d == a = (a,b+1) : incrementKey d _rest
            | otherwise = (a,b) : incrementKey d _rest
...
winner scores player1 player2 = if (Map.findWithDefault 0 player2 scores) > 
...
freqs xs = Map.fromList[(i, count i xs) | i <- nub xs]
                 where count s ss = length (filter ( == s ) xs)
...
transfer from to amount bank 
       | Map.notMember from bank || Map.notMember to bank || amount <= 0 = bank
       | (Map.findWithDefault 0 from bank) < amount = bank
       | otherwise = Map.adjust (\x -> x + amount) to 
                    (Map.adjust (\x -> x - amount) from bank)
