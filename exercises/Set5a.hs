...
toList Empty = []
toList (Pair k v _rest) = (k,v) : toList _rest

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList ((k,v) : _rest) = Pair k v (fromList _rest)
...
toNat x 
  | x < 0 = Nothing
  | otherwise = Just (toNatH x)

toNatH :: Int -> Nat
toNatH 0 = Zero
toNatH 1 = PlusOne Zero
toNatH x = PlusOne (toNatH (x-1))
...
prettyPrint End = ""
prettyPrint (O str) = prettyPrint str ++ "0"
prettyPrint (I str) = prettyPrint str ++ "1"

fromBin :: Bin -> Int
fromBin n = fromBinH n 0

fromBinH :: Bin -> Int -> Int 
fromBinH (O End) _ = 0
fromBinH (I End) i = 2^i
fromBinH (O str) i = fromBinH str (i+1)
fromBinH (I str) i = 2^i + fromBinH str (i+1)
