...
fmap2 f l = fmap (\x -> fmap f x) l

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f l = fmap (\x -> fmap2 f x) l
...
instance Functor Result where
  ...
  fmap _ (Failure string) = Failure string
...
count c obj = length (filter (\x -> x == c) (toList $ obj))
...
inBoth a b = filter (\x -> elem x (toList a)) (toList b)
