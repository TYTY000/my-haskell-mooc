...
interpret _ _ [] = []
interpret x y commands = evaluate sx sy (head getHead) : interpret sx sy (tail getHead)
                                   where getHead = dropWhile direction commands
                                         getDir = takeWhile direction commands
                                         sx = sum (map getX getDir) + x
                                         sy = sum (map getY getDir) + y

getX :: String -> Int
getX x = case x of "left" -> -1
                   "right" -> 1
                   otherwise -> 0

getY :: String -> Int
getY y = case y of "down" -> -1
                   "up" -> 1
                   otherwise -> 0
...
evaluate x _ "printX" = show x
evaluate _ y "printY" = show y
