...
checkNumber (a,b) = if anyNumeric then Nothing
                    else Just (a,b)
                    where anyNumeric = checkNumber' a || checkNumber' b

checkNumber' :: String -> Bool
checkNumber' [] = False
checkNumber' (x:xs) = if elem x numbers then True
                      else checkNumber' xs
                        where numbers = ['0','1'..'9']
...
checkCapitals (for,sur) = if (upperHead for) && (upperHead sur)
                          then Just (for, sur)
                          else Nothing
                            where upperHead = isUpper.head
...
winner scores player1 player2 = do
          ...
          test (player1,a) (player2,b)

test :: (String, Maybe Int) -> (String, Maybe Int) -> Maybe String
test (p1,(Just s1)) (p2,(Just s2)) = if s1 >= s2 then Just p1 else Just p2
test _ _ = Nothing
...
selectSum xs (i:is) = do
            ...
            b <- selectSum xs is
            return (a+b)
-- safeIndex xs i >>= \a -> selectSum xs is >>= \b -> Just (a+b)
...
paren c = do s <- get
             if (s == -1)
             then return ()
             else modify (\s -> s + if c == '(' then 1 else if c == ')' then -1 else 0)
...
count x = do s <- get
             let mod = Map.insertWith (+) x 1 $ Map.fromList s
             put (Map.toList mod)
