...
appendAll refstr strs = forM_ strs (\x -> modifyIORef refstr (++x))
...
hSelectLines file nums = do 
      line <- hFetchLines file
      let res = map (\n -> line !! (n-1)) nums
      -- IO [String] cannot be !! indexed
      return res
...
interact' f state = do
    line <- getLine
    let (flag, message, newstate) = f (line, state) 
    putStrLn message
    if flag 
